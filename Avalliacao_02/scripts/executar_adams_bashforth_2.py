"""Programa separado para executar o método: ab2.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Executa o método ab2 para as malhas da avaliação.")
    ...
    requested_c = max_stable_c("ab2") if args.C is None else args.C
    used_c, limited = limit_c("ab2", requested_c)
    if limited:
        print(f"[ajuste] método=ab2 C solicitado={requested_c:g} excede o limite; usando C={used_c:g}.")

    for N in MESHES:
        print(f"Executando método=ab2, Nx=Ny={N}, C={used_c:g}")
