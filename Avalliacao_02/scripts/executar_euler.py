"""Programa separado para executar o método: euler.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Executa o método euler para as malhas da avaliação.")
    ...
    for N in MESHES:
        print(f"Executando método=euler, Nx=Ny={N}, C={args.C:g}")
        result = run_simulation(method="euler", Nx=N, Ny=N, C=args.C, xp=xp, using_cuda=using_cuda, cfg=cfg)
